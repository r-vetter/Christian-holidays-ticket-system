	...
	*/
	...
	class OmniKassa
	{
		...
		public function createForm()
		{
			...
			$sSubmitButton = ($this->sButtonImage ? '<input type="image" value="' . htmlspecialchars($this->sButtonLabel) . '" src="' . htmlspecialchars($this->sButtonImage) . '"' . ($this->iButtonImageWidth ? ' width="' . htmlspecialchars($this->iButtonImageWidth) . '"' : '') . ($this->iButtonImageHeight ? ' height="' . htmlspecialchars($this->iButtonImageHeight) . '"' : '') . '>' : '<input type="submit" class="btn btn-info" value="' . htmlspecialchars($this->sButtonLabel) . '">');

			// Generate HTML form
			$sHtml = '<form method="post" action="' . htmlspecialchars($this->sAquirerUrl) . '" name="checkout"><input type="hidden" name="Data" value="' . htmlspecialchars($sData) . '"><input type="hidden" name="InterfaceVersion" value="HP_1.0"><input type="hidden" name="Seal" value="' . htmlspecialchars($sHash) . '">' . $sSubmitButton . '</form>';
			return $sHtml;
		}
