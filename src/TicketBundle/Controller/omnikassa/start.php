<?php
	...
	*/
	...
	$aPaymentMethods = array(); // Laat de klant zelf een betaalmethode kiezen op de betaalpagina van de Rabo OmniKassa.
	...
	$oOmniKassa->setReportUrl($aSettings['website_url'] . $this->generateUrl('subscriber_frondend_confirmation',array('id' => $item->getId()))); // Mag geen additionele parameters bevatten
	$oOmniKassa->setReturnUrl($aSettings['website_url'] . $this->generateUrl('subscriber_frondend_confirmation',array('id' => $item->getId()))); // Mag geen additionele parameters bevatten
	...
	// order kunt terug vinden en de status kunt bijwerken. Houdt er rekening mee dat bij één $sOrderId mogelijk meerdere betaalverzoeken 
